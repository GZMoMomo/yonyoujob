...
首先接入源头数据，接着通过ETL开发工作，再把这些数据加工成数据模型，然后根据报表层对数据的需求，把模型进行二次汇总，最后把汇总后的结果存到如MySQL，报表从二次汇总后的数据查询。
...
### 2022.7.22 第二十一天
#### 上午
初试数据建模（根据一个报表需求样式，设计数据库字段结构）
#### 下午
对数据建模上手一点点

### 2022.7.25 第二十二天
#### 上午
完成数据建模（根据报表样式，自己设计需要的字段）
#### 下午
模仿看板设计，手动做一个。
...
字节跳动项目
2022.8.4~2022.8.5  
医联体信息化平台建设 抽取数据到ODS层 全增

2022.8.8~2022.8.9
根据医院提供的指标，从原表（事实表）开始搭建数据仓库，stg层 ods层 dw层 dm层，制作bi报表。
